...
On macOS, `brew --prefix bash` is preferred when available; otherwise `/bin/bash` is used. On Ubuntu, inspect `echo "$XDG_SESSION_TYPE"` to distinguish Wayland from X11. On Windows, use MSYS2 UCRT64 Bash and keep Unreal paths in native form when passing them to Windows applications.
