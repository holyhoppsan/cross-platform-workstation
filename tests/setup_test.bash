...
assert_contains "$agent_setup" "'npm:@narumitw/pi-usage' 'npm:pi-mcp-adapter'" 'macOS Pi setup declares the default Pi plugins'
assert_contains "$agent_setup" "'npm:pi-mcp-adapter'" 'macOS Pi setup declares the MCP adapter plugin'
assert_contains "$agent_setup" '"$command_name" list' 'macOS Pi setup checks global plugins before installing'
assert_contains "$agent_setup" '"$command_name" install "$pi_plugin"' 'macOS Pi setup installs the default usage plugin through Pi'
...
assert_contains "$common_ps1" "@('npm:@narumitw/pi-usage', 'npm:pi-mcp-adapter')" 'Windows Pi setup declares the default Pi plugins'
assert_contains "$common_ps1" "'npm:pi-mcp-adapter'" 'Windows Pi setup declares the MCP adapter plugin'
assert_contains "$common_ps1" '& $agentCommand list' 'Windows Pi setup checks global plugins before installing'
assert_contains "$common_ps1" '& $agentCommand install $plugin' 'Windows Pi setup installs the default usage plugin through Pi'
