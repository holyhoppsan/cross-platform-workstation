...
assert_contains "$wezterm_config" "msys2_bash = 'C:/msys64/usr/bin/bash.exe'" 'WezTerm launches MSYS2 Bash on Windows'
assert_contains "$wezterm_config" "MSYSTEM = 'UCRT64'" 'WezTerm sets the UCRT64 environment on Windows'
...
assert_contains "$wezterm_config" "key = 'e', mods = 'LEADER'" 'WezTerm has Yazi new-pane binding'
assert_contains "$wezterm_config" "spawn_interactive_bash_command('y')" 'WezTerm launches Yazi new-pane helper in interactive Bash'
assert_contains "$wezterm_config" "key = 'e', mods = 'LEADER|SHIFT'" 'WezTerm has shifted Yazi current-pane binding'
...
assert_contains "$wezterm_config" "key = 'v', mods = 'CTRL', action = act.PasteFrom 'Clipboard'" 'WezTerm reserves Ctrl+V for terminal clipboard paste'
...
assert_contains "$wezterm_doc" 'not part of the AI agent launcher' 'WezTerm docs preserve Yazi non-agent boundary'
